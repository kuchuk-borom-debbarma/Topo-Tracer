...
await tracer.trace('process-payment', async (span) => {
  ...
  // Log with importance level (0 = Critical, 3 = Low, omitting the data object)
  tracer.log('Retrying connection', 1);

  // Log with metadata and importance level
  tracer.log('Database sync complete', { status: 'success' }, 2);
});
```
