 ...
 * Rows are keyed by user, trace, and node id.
 * Uses ReplacingMergeTree to keep only the latest materialized version per node.
 * Materialization writes replacement rows with incrementing materialized_at_ms.
 */
 ...
 * Rows are keyed by user, trace, and edge id.
 * Denormalizes from/to node flow orders to support bounded projection queries without joins.
 */
 ...
 * Keyed by user and trace id.
 * Replaces old summary versions as materialization progress expands.
 */
 ...
 * Keyed by user and trace to allow fine-grained resume of materialization.
 * Includes deterministic tie-breakers (id and type) for events sharing timestamps.
 */
