...
export const LAYOUT = {
  ...
  BLOCK_PAD: 10,          // block internal padding top/bottom (px)
  BLOCK_HEADER_H: 42,     // block header height (px)
  COL_W: 240,             // block card width (px)
  COL_GAP: 60,            // gap between depth columns (for arrows)
...
function colX(depth: number): number {
  return depth * (LAYOUT.COL_W + LAYOUT.COL_GAP);
}
...
export function computeLayout(
  ...
  function resolveNodeId(nodeId: string): string | null {
    ...
    // Snap to visible ancestor via ancestryPath
    const node = dedupedNodes.find((n) => n.id === nodeId);
    if (node) {
  ...
  for (const edge of edges) {
    ...
    const fromCid = fromBlockId ? blockContainerMap.get(fromBlockId) : undefined;
    const toCid   = toBlockId   ? blockContainerMap.get(toBlockId)   : undefined;
