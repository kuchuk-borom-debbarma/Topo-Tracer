...
function TraceViewer() {
  ...
  useEffect(() => {
    if (data && data.nodes.length > 0) {
      // Reconstruct containers safely
      const containers = data.containers || Array.from(new Set(data.nodes.map(n => n.containerId))).map(id => ({ id, name: id, containerType: 'unknown', createdAtLocal: new Date(), createdAtRemote: new Date() }));
          ...
          >
            ...
            <Panel position="top-right" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', background: 'var(--bg-surface-elevated)', padding: '0.5rem 1rem', borderRadius: 'var(--border-radius-sm)', border: '1px solid var(--glass-border)' }}>
              <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginRight: '0.5rem' }}>
                Resolution: {depthFilter === 0 ? 'Macro (Containers)' : depthFilter === 2 ? 'Meso (Components)' : 'Micro (Code)'}
              </span>
              <button className="btn" onClick={() => setDepthFilter(prev => Math.max(prev - 2, 0))} disabled={depthFilter === 0}>
                <ZoomOut size={16} />
              </button>
              <button className="btn" onClick={() => setDepthFilter(prev => Math.min(prev + 2, 4))} disabled={depthFilter >= 4}>
                <ZoomIn size={16} />
              </button>
            </Panel>
          </ReactFlow>
