...
function createColumn(table, columnDef, depth, parent) {
	...
	if (!id) throw new Error(resolvedColumnDef.accessorFn ? `Columns require an id when using an accessorFn` : `Columns require an id when using a non-string header`);
	let column = {
		id: `${String(id)}`,
		accessorFn,
		parent,
...
function createTable(options) {
	...
	if (options.debugAll || options.debugTable) console.info("Creating Table Instance...");
	...
	const coreInstance = {
		...
		getRow: (id, searchAll) => {
			...
			if (!row) {
				...
				if (!row) throw new Error(`getRow could not find row with ID: ${id}`);
			}
			return row;
		},
		...
		getColumn: (columnId) => {
			...
			if (!column) console.error(`[Table] Column with id '${columnId}' does not exist.`);
			return column;
		}
	};
...
function getFacetedMinMaxValues() {
	return (table, columnId) => memo(() => {
		...
		for (const value of uniqueValues) if (value < facetedMinValue) facetedMinValue = value;
...
function getFacetedRowModel() {
	return (table, columnId) => memo(() => [
		...
		if (!preRowModel.rows.length || !(columnFilters != null && columnFilters.length) && !globalFilter) return preRowModel;
	...
	return (table) => memo(() => [table.getState().grouping, table.getPreGroupedRowModel()], (grouping, rowModel) => {
		...
		const groupUpRecursively = function(rows, depth, parentId) {
			...
			if (depth >= existingGrouping.length) return rows.map((row) => {
				...
				if (row.subRows) row.subRows = groupUpRecursively(row.subRows, depth + 1, row.id);
				return row;
			});
			const columnId = existingGrouping[depth];
			const rowGroupsMap = groupBy(rows, columnId);
			return Array.from(rowGroupsMap.entries()).map((_ref, index) => {
					...
					getValue: (columnId) => {
						if (existingGrouping.includes(columnId)) {
							...
							if (groupedRows[0]) {
								...
								row._valuesCache[columnId] = (_groupedRows$0$getVal = groupedRows[0].getValue(columnId)) != null ? _groupedRows$0$getVal : void 0;
							}
							return row._valuesCache[columnId];
						}
						if (row._groupingValuesCache.hasOwnProperty(columnId)) return row._groupingValuesCache[columnId];
